...
We assume the asset price follows a lognormal random walk in a frictionless, time continuum setting. The expected return equals the risk-free interest rate which is taken to be constant over time. In addition, the asset does not pay dividends over the life of the option. This  method is called "risk-neutral" valuation.  -->
...
<!-- 
\begin{equation*}
V_{AC_{(T)}} = v_{AC}(T, S_{(T)}) = S_{(T)} \mathbbm{1}_{[S_{(T)} > K]} 
\end{equation*}  -->
...
$\Phi(d_{1})$ is a measure by which the discounted expected value of contingent receipt of the asset exceeds the present value of the asset.
...
<!-- \begin{equation*}
    V_{AP_{(T)}} = v_{AP}(T, S_{(T)}) = S_{(T)}\mathbbm{1}_{\{S_{(T)} < K\}} 
\end{equation*} -->
...
<!-- \begin{equation*}
    V_{CC_{(T)}} = v_{CC} (T, S_{(T)}) = \mathbbm{1}_{[S_{(T)}>K]} 
\end{equation*} -->
...
<!-- \begin{equation*}
    V_{CP_{(T)}} = v_{CP}(T, S_{(T)}) = \mathbbm{1}_{\{S_{(T)} < K\}} 
\end{equation*} -->
